...
function generateGrpcWebClientManually(schema: Schema, file: DescFile) {
  ...
  // Generate imports
  content += `import { LoggingInterceptorConnect } from "../../../common/grpc_web";\n`;

  // Import the generated gRPC-web service client
  for (const service of file.services) {
    content += `import { ${service.name} } from "./service_pb";\n`
    content += `import { createConnectTransport } from "@connectrpc/connect-web";\n`
    content += `import { createClient, Interceptor } from "@connectrpc/connect";\n`
  }
  ...
  content += `import { GroupHeaderInterceptorConnect } from "../../../common/groupHeaderInterceptor";\n`;
...
function generateServiceClientString(service: DescService, file: DescFile): string {
  ...
  const serviceClientName = `${serviceName}`;
  ...
  content += `  private _service: ReturnType<typeof createClient<typeof ${serviceClientName}>>;`
  ...
  content += "    this._interceptors = interceptors ? [...interceptors, LoggingInterceptorConnect] : [LoggingInterceptorConnect];\n";
  content += "\n";
  ...
  content += "      (i: any) => i.__group !== undefined\n";
  content += "    );\n";
  content += "\n";
  ...
  content += "    const groupInterceptor = GroupHeaderInterceptorConnect(group);\n";
  content += "\n";
...
function generateStreamingMethodString(
  ...
  content += "    this._interceptors.forEach((interceptor: any) => {\n"
  content += "      if (interceptor.__group !== undefined) {\n";
  content += "        // Extract group header from GroupHeaderInterceptor\n";
  content += "        metadata['x-group'] = interceptor.__group;\n";
  content += "      }\n";
  content += "    });\n";
  content += "\n";
