...
func generateClientFile(p *protogen.Plugin, f *protogen.File, svc *protogen.Service) error {
	...
	g.P("\t}")
	g.P()
	g.P("\t// attempt to load credentials using discovery hierarchy (environment variable or default file)")
	g.P("\tif creds, err := ", generate.APICredentialsPkg.Ident("FindCredentials"), "(); err == nil {")
