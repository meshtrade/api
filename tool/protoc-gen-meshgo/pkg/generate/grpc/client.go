...
func generateOptionsFile(p *protogen.Plugin, f *protogen.File, svc *protogen.Service) error {
	...
	optionsG.P("//")
	optionsG.P("// IMPORTANT: When using API key authentication, you must also specify a group ID")
	...
	optionsG.P("//\t    WithGroup(\"your-group-id\"),")
	optionsG.P("//\t)")
	...
	optionsG.P("//")
	optionsG.P("// IMPORTANT: When using access token cookie authentication, you must also specify a group ID")
	optionsG.P("// using WithGroup() as group information cannot be loaded from credentials file.")
	optionsG.P("//")
	...
	optionsG.P("// WithGroup configures the group ID for all API requests made by this client.")
	optionsG.P("// The group ID is required for public API calls and determines the authorization context")
	...
	optionsG.P("// When loading from credentials file via MESH_API_CREDENTIALS, the group ID")
	optionsG.P("// is automatically loaded and this option is optional (but will override the file value).")
	optionsG.P("//")
	optionsG.P("// Parameter:")
	optionsG.P("//   - groupID: The group identifier string")
	optionsG.P("//")
	optionsG.P("// Example:")
	optionsG.P("//")
	optionsG.P("//\tclient, err := New", svc.GoName, "GRPCClient(")
	optionsG.P("//\t    WithAPIKey(\"your-api-key\"),")
	optionsG.P("//\t    WithGroup(\"01ABCDEF123456789\"),")
	optionsG.P("//\t)")
	optionsG.P("func WithGroup(groupID string) ", optionTypeName, " {")
	optionsG.P("\treturn func(c *", clientStructName, ") {")
	optionsG.P("\t\tc.groupID = groupID")
	optionsG.P("\t}")
	optionsG.P("}")
	optionsG.P()
...
func generateClientFile(p *protogen.Plugin, f *protogen.File, svc *protogen.Service) error {
	...
	g.P("//")
	g.P("// Features:")
	g.P("//   - Automatic authentication via API key or access token cookies with group ID support")
	...
	g.P("\tif creds, err := common.CredentialsFromEnvironment(); err == nil {")
	g.P("\t\tclient.apiKey = creds.APIKey")
	g.P("\t\tclient.groupID = creds.GroupID")
	g.P("\t}")
	g.P()
	...
	g.P("//   - Group ID must be set for all public API calls")
	g.P("//")
	...
	g.P("//")
	g.P("// Returns:")
	g.P("//   - nil: If authentication and group ID are properly configured")
	g.P("//   - error: If authentication method or group ID is missing")
	g.P("func (c *", clientStructName, ") validateAuth() error {")
	g.P("\tif c.apiKey == \"\" && c.accessTokenCookie == \"\" {")
	g.P("\t\treturn errors.New(\"neither api key nor access token cookie set. set credentials via MESH_API_CREDENTIALS file, or use WithAPIKey/WithAccessTokenCookie options\")")
	g.P("\t}")
	g.P("\tif c.groupID == \"\" {")
	g.P("\t\treturn errors.New(\"group id not set. set via MESH_API_CREDENTIALS file or WithGroup option\")")
	g.P("\t}")
	g.P("\treturn nil")
	g.P("}")
	g.P()
	...
	g.P("// based on the configured authentication method (API key takes precedence over access token cookie).")
	g.P("//")
	g.P("// Headers Added:")
	g.P("//   - API Key: \"Authorization: Bearer <api-key>\" header")
	g.P("//   - Access Token Cookie: \"Cookie: AccessToken=<token>\" header")
	g.P("//   - Group ID: \"x-group-id: <group-id>\" header (always included)")
	g.P("//")
