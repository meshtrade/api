...
function calculateCommonImportPath(file: DescFile): string {
  // Calculate the relative path from the service file to the common directory
  // Example: "meshtrade/testing/ledger/token_tap/v1/service.proto" should go up 4 levels
  // to "meshtrade/common"
  const protoPath = file.name; // e.g., "meshtrade/testing/ledger/token_tap/v1/service"
  const parts = protoPath.split('/');

  // Count levels from service file to meshtrade directory
  // We need to go up from "v1" to "meshtrade" (excluding the file name itself)
  // For "meshtrade/iam/api_user/v1/service" we go up 3 levels (v1, api_user, iam)
  // For "meshtrade/testing/ledger/token_tap/v1/service" we go up 4 levels (v1, token_tap, ledger, testing)

  // Count how many directory parts are in the path (excluding the file name "service")
  // and subtract 1 for "meshtrade"
  const levelsToMeshtrade = parts.length - 2; // -1 for "meshtrade", -1 for the file name

  // Generate the relative path: "../" repeated for each level, then "../common"
  return new Array(levelsToMeshtrade).fill('..').join('/') + '/common';
}
...
function generateGrpcWebClientManually(schema: Schema, file: DescFile) {
  ...
  // Calculate relative path to common directory based on file depth
  const commonPath = calculateCommonImportPath(file);

  // Generate imports
  content += `import { LoggingInterceptor } from "${commonPath}/grpc_web";\n`;
  ...
  content += `import { ConfigOpts, getConfigFromOpts } from "${commonPath}/config";\n`;
  content += `import { GroupHeaderInterceptor } from "${commonPath}/groupHeaderInterceptor";\n`;
  content += `\n`;
  content += `/**\n`;
  content += ` * Validates that a request object is not null or undefined.\n`;
  content += ` * @param {any} request - The request object to validate\n`;
  content += ` * @throws {Error} If the request is null or undefined\n`;
  content += ` */\n`;
  content += `function validateRequest(request: any): void {\n`;
  content += `  if (!request) {\n`;
  content += `    throw new Error('Request cannot be null or undefined');\n`;
  content += `  }\n`;
  content += `}\n`;
  content += `\n`;
