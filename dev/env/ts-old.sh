...
handle_error() {
  ...
  local line_number=$1
  echo
  echo "❌ ERROR in $(basename "$0") on line $line_number: TypeScript (Legacy) environment check failed!"
  exit "$exit_code"
}

cd "$ROOT_DIR"

echo "🔷 TypeScript (Legacy) Environment Check"
echo "========================================="
...
if [[ ! -d "$ROOT_DIR/ts-old/src/meshtrade" ]]; then
    echo "❌ ERROR: TypeScript (Legacy) source directory not found at ts-old/src/meshtrade/"
    echo "   Please run code generation first:"
    echo "   ./dev/tool.sh generate --targets=ts-old"
    exit 1
fi
...
# Check protoc-gen-mesh_ts_old plugin
PLUGIN_DIR="$ROOT_DIR/tool/protoc-gen-mesh_ts_old"
if [[ -d "$PLUGIN_DIR" ]] && [[ ! -d "$PLUGIN_DIR/node_modules" ]]; then
    echo "⚠️  WARNING: protoc-gen-mesh_ts_old dependencies not found"
    echo "   This will be installed automatically during code generation"
fi

echo "✅ TypeScript (Legacy) environment validated successfully!"
