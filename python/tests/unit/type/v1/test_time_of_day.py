...
class TestNewTimeOfDay:
    ...
    @pytest.mark.parametrize("hours,minutes,seconds,nanos,expected_error", [
...
class TestNewTimeOfDayFromPythonTime:
    ...
    @pytest.mark.parametrize("python_time_obj,expected_hours,expected_minutes,expected_seconds,expected_nanos", [
...
class TestNewTimeOfDayFromDatetime:
    ...
    @pytest.mark.parametrize("datetime_obj,expected_hours,expected_minutes,expected_seconds,expected_nanos", [
...
class TestNewTimeOfDayFromTimedelta:
    ...
    @pytest.mark.parametrize("timedelta_obj,expected_error", [
    ...
    def test_new_time_of_day_from_timedelta(self, timedelta_obj, expected_error):
        ...
        if expected_error:
            ...
            total_seconds_result = (result.hours * 3600 + result.minutes * 60 + 
                                  result.seconds + result.nanos / 1e9)
...
class TestIsValid:
    ...
    def test_is_valid(self, time_obj, expected):
        ...
        assert is_valid(time_obj) == expected
...
class TestIsMidnight:
    ...
    def test_is_midnight(self, time_obj, expected):
        """Test midnight check."""
        assert is_midnight(time_obj) == expected
...
class TestTimeOfDayToPythonTime:
    ...
    def test_invalid_conversions(self):
        ...
        with pytest.raises(ValueError, match="TimeOfDay object is None"):
            time_of_day_to_python_time(None)
...
class TestTimeOfDayToDatetimeWithDate:
    ...
    def test_invalid_conversions(self):
        ...
        # None time
        with pytest.raises(ValueError, match="TimeOfDay object is None"):
            time_of_day_to_datetime_with_date(None, proto_date)

        # None date
        with pytest.raises(ValueError, match="Date object is None"):
            time_of_day_to_datetime_with_date(proto_time, None)

        # Incomplete date
        with pytest.raises(ValueError, match="Date must be complete"):
            time_of_day_to_datetime_with_date(proto_time, incomplete_date)
...
class TestTimeOfDayToTimedelta:
    ...
    @pytest.mark.parametrize("time_obj,expected_seconds", [
        (None, 0),
