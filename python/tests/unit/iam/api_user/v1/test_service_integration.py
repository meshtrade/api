...
class TestApiUserServiceCredentialFiles:
    ...
    def test_valid_credential_file(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            valid_credentials = {"api_key": "test-api-key-from-file", "group": "groups/01ARZ3NDEKTSV4RRFFQ69G5FAV"}
            ...
            options = ServiceOptions(url="localhost", port=9999, timeout=timedelta(milliseconds=100))
        ...
        for case in test_cases:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                try:
                    ...
                    assert any(word in str(e).lower() for word in ["api_key", "group", "json", "credentials", "parse"])
    ...
    def test_credential_file_overridden_by_options(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            file_credentials = {"api_key": "file-api-key", "group": "groups/01FILECREDENTIALSTEST123"}
    ...
    def test_credential_file_permissions(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            # Create a credentials file
            valid_credentials = {"api_key": "permission-test-key", "group": "groups/01PERMISSIONTEST12345"}
    ...
    def test_credential_discovery_hierarchy(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            env_credentials = {"api_key": "env-file-api-key", "group": "groups/01ENVFILECREDENTIALS123"}
    ...
    def test_credential_environment_variable_name(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            valid_credentials = {"api_key": "env-var-test-key", "group": "groups/01ENVVARTEST123456789"}
