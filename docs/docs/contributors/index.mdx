...
# 2. Verify Java version is exactly 21 (not 24 or other versions)
java -version
# Expected output: openjdk version "21.0.x" 2025-xx-xx

# 3. Verify Maven can find Java 21
mvn -version
...
```bash
# Add this line to ensure Java 21 is always available
export PATH="/opt/homebrew/opt/openjdk@21/bin:$PATH"
```
...
# 3. Configure PATH for Java (CRITICAL)
echo 'export PATH="/opt/homebrew/opt/openjdk@21/bin:$PATH"' >> ~/.zshrc
source ~/.zshrc
...
cd ../..
buf generate --template scripts/code-generation/buf/buf.gen.yaml --include-imports proto/meshtrade/iam/api_user/v1/service.proto
