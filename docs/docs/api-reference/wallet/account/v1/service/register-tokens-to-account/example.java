...
import co.meshtrade.api.wallet.account.v1.Service.RegisterTokensToAccountRequest;
import co.meshtrade.api.wallet.account.v1.Service.RegisterTokensToAccountResponse;

import java.util.Optional;

public class RegisterTokensToAccountExample {
    public static void main(String[] args) {
        ...
        try (AccountService service = new AccountService()) {
            ...
            RegisterTokensToAccountRequest request = RegisterTokensToAccountRequest.newBuilder()
            ...
            RegisterTokensToAccountResponse response = service.registerTokensToAccount(request, Optional.empty());

            // The response contains a ledger transaction reference to monitor
            System.out.println("RegisterTokensToAccount submitted: " + response.getLedgerTransaction());
            System.out.println("Monitor the transaction state to confirm completion");
        } catch (Exception e) {
            System.err.println("RegisterTokensToAccount failed: " + e.getMessage());
            e.printStackTrace();
        }
    }
}
