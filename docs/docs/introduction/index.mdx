...
These roles will be to the API User **in the group** that will own it - the top level group of the client that you are logged in as.
...
:::info
See **[Group Ownership Structure](./architecture/group-ownership)** for more information on group structures in Mesh.
:::
...
:::danger
Keep your API credentials file **secure**! Never commit this credentials file version control!
:::

3. The new API User is inactive on creation. Select its row in the table and click on the
...
![Create API User Dialog Add Roles](./activate-api-user-success.png)
...
With Maven dependency declaration:
```xml
<dependency>
...
👉 **[Permissions Guide](/docs/architecture/role-based-access)** - Master RBAC, roles, and permission management
