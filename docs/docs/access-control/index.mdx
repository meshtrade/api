...
Clients represent compliance-verified legal entities (natural person, company, fund, or trust) within the group hierarchy. Each client is owned by a group. A client's verification status (`VERIFIED`, `PENDING`, etc.) is what methods with `verification_status: VERIFICATION_STATUS_VERIFIED` check against.

```mermaid
graph TD
    subgraph "Client Types"
        Company["Company Client<br/>Type: Company<br/>Status: Verified"]
        Fund["Fund Client<br/>Type: Fund<br/>Status: Verified"]
        NaturalPerson["Individual Client<br/>Type: Natural Person<br/>Status: Verified"]
        Trust["Trust Client<br/>Type: Trust<br/>Status: Pending"]
    end

    style Company fill:#e1f5fe
    style Fund fill:#f3e5f5
    style NaturalPerson fill:#fff3e0
...
3. **Entity identification** — hash the API key and retrieve the corresponding API User
