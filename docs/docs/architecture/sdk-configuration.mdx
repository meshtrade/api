...
All Meshtrade SDK clients are generated from the same template and provide consistent configuration options across Go, Python, and TypeScript implementations. You can configure:
...
```typescript title="advanced-client-config.ts"
import {
  ApiUserServiceGRPCClient,
  ClientOptions,
  ListApiUsersRequest,
} from '@meshtrade/api/iam/api_user/v1';

async function main() {
  // Option 1: Minimal client (uses MESH_API_CREDENTIALS environment variable)
  const options1: ClientOptions = {};
  const client1 = new ApiUserServiceGRPCClient(options1);
  ...
  const client2 = new ApiUserServiceGRPCClient(options2);
  ...
  const client3 = new ApiUserServiceGRPCClient(options3);

  // Option 4: Custom timeout settings
  const options4: ClientOptions = {
    url: 'api.mesh.dev',
    port: 443,
    tls: true,
    timeout: 30000, // 30 seconds in milliseconds
  };
  const client4 = new ApiUserServiceGRPCClient(options4);

  try {
    // All clients have the same interface and can be used identically
    // Example: List API users with any configured client
    const response = await client1.listApiUsers(new ListApiUsersRequest());
    console.log(`Found ${response.apiUsers.length} API users`);
  } catch (error) {
