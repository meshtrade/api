...
A complex structure with multiple legal entities in a hierarchical relationship. This example demonstrates how institutional fund managers organize multiple investment vehicles, each with distinct legal entities, compliance requirements, and operational boundaries.

<div style={{display: 'grid', gap: '2rem', marginTop: '1.5rem'}}>

#### Legal Entity Hierarchy

This diagram shows the organizational structure and legal entity relationships within the fund management structure.

```mermaid
graph TD
    subgraph "Legal Entity Structure"
        MeshRoot["🏢 Mesh Platform
        groups/MESH_ROOT"]
        ...
        Type: Natural Person | Status: Verified"]
        ...
        style ManagementGroup fill:#e3f2fd
        style AlphaFundGroup fill:#f3e5f5
        style BetaFundGroup fill:#e8f5e8
        style InvestorGroup fill:#fff3e0
    end
```

**Legal Structure:** The management company (Goldman Asset Management) owns and operates multiple fund structures. Each legal entity undergoes independent KYC verification and maintains separate compliance obligations.

---

#### User Access & Authentication

This diagram focuses on user access patterns, authentication methods, and role assignments across the fund structure.

```mermaid
graph TD
    subgraph "Access & Authentication Structure"
        ManagementGroup["🏛️ Goldman Asset Management
        groups/GOLDMAN_MGMT"]
        ...
        🔐 User Authentication
        Roles: [COMPLIANCE_ADMIN, REPORTING_ADMIN]"]

        FundManager["👤 Portfolio Manager
        users/PORTFOLIO_MGR
        🔐 User Authentication
        Roles: [TRADING_ADMIN]"]

        RiskSystem["🔑 Risk Management System
        api_users/RISK_SYSTEM
        🔐 API Key Authentication
        Roles: [TRADING_VIEWER, REPORTING_VIEWER]"]

        InvestorPortal["👤 Investor User
        users/INVESTOR_USER
        🔐 User Authentication
        Roles: [TRADING_VIEWER, WALLET_VIEWER]"]
        ...
        style ComplianceOfficer fill:#ffebee
        style FundManager fill:#e8f5e8
        style RiskSystem fill:#f3e5f5
        style InvestorPortal fill:#fff3e0
    end
```
**Access Patterns:** The system supports two types of users: human users who authenticate through SSO, credentials, or other user authentication methods, and API users who authenticate using API keys. Both user types operate within the same authorization framework but with different authentication mechanisms suitable for their usage patterns.

---

#### Financial Account Structure

This diagram illustrates the financial account ownership and fund flow relationships within the structure.

```mermaid
graph TD
    subgraph "Financial Account Structure"
        AlphaFundGroup["💼 Alpha Hedge Fund LP
        groups/ALPHA_FUND"]
        ...
        InvestorAccount -.->|"Subscription Funds"| AlphaAccount
        InvestorAccount -.->|"Investment Capital"| BetaAccount

        style AlphaAccount fill:#e8f5e8
        style BetaAccount fill:#e3f2fd
        style InvestorAccount fill:#fff3e0
    end
```

**Financial Flow:** Investment capital flows from investor subscription accounts into fund master accounts, maintaining clear ownership boundaries while enabling complex fund operations.

</div>

**Key Characteristics:**
- **Multiple Legal Entities**: Management company, two funds, and individual investor with separate legal status
- **Distinct Role Boundaries**: Each client defines different available roles based on operational requirements
- **Compliance Segregation**: Independent legal entities with separate verification and regulatory obligations
- **Hierarchical Legal Structure**: Investment manager legally owns fund structures, funds maintain individual investor relationships
- **Segregated Financial Operations**: Clear account ownership with compliant fund flow management
...
The [Client](/api/docs/api-reference/compliance/client/v1/type/client/) protobuf includes a critical `roles` field that establishes role boundaries:
