...
// AccountReportServiceClientInterface is a gRPC service for the AccountReportService service.
...
type accountReportService struct {
	*grpc.BaseGRPCClient[AccountReportServiceClient]
}

// ensure accountReportService implements the AccountReportServiceClientInterface interface
var _ AccountReportServiceClientInterface = &accountReportService{}

// NewAccountReportService creates and initializes the AccountReportService service.
...
//
// Returns:
//   - AccountReportServiceClientInterface: Configured service instance
//   - error: Configuration or connection error
func NewAccountReportService(opts ...config.ServiceOption) (AccountReportServiceClientInterface, error) {
	base, err := grpc.NewBaseGRPCClient(
		AccountReportServiceServiceProviderName,
...
func (s *accountReportService) GetAccountReport(ctx context.Context, request *GetAccountReportRequest) (*AccountReport, error) {
	return grpc.Execute(s.Executor(), ctx, "GetAccountReport", request, func(ctx context.Context) (*AccountReport, error) {
		return s.GrpcClient().GetAccountReport(ctx, request)
	})
}

// GetExcelAccountReport executes the GetExcelAccountReport RPC method with automatic
// client-side validation, timeout handling, distributed tracing, and authentication.
func (s *accountReportService) GetExcelAccountReport(ctx context.Context, request *GetExcelAccountReportRequest) (*GetExcelAccountReportResponse, error) {
	return grpc.Execute(s.Executor(), ctx, "GetExcelAccountReport", request, func(ctx context.Context) (*GetExcelAccountReportResponse, error) {
		return s.GrpcClient().GetExcelAccountReport(ctx, request)
	})
}
