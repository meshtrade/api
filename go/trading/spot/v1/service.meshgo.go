...
func NewSpotService(opts ...ServiceOption) (SpotServiceClientInterface, error) {
	...
	service.conn = conn
	service.grpcClient = NewSpotServiceClient(conn)
...
// Example:
//
//	resp, err := service.GetSpot(ctx, &GetSpotRequest{
...
func (s *spotService) Close() error {
	if s.conn != nil {
		return s.conn.Close()
	}
	return nil
...
func (s *spotService) Group() string {
	return s.group
}
...
func (c *spotService) validateAuth() error {
	if c.apiKey == "" {
...
func (c *spotService) authInterceptor() grpc.UnaryClientInterceptor {
