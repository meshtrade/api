...
func NewLimitOrderService(opts ...ServiceOption) (LimitOrderServiceClientInterface, error) {
	...
	service.conn = conn
	service.grpcClient = NewLimitOrderServiceClient(conn)
...
// Example:
//
//	resp, err := service.GetLimitOrder(ctx, &GetLimitOrderRequest{
...
func (s *limitOrderService) Close() error {
	if s.conn != nil {
		return s.conn.Close()
	}
	return nil
...
func (s *limitOrderService) Group() string {
	return s.group
}
...
func (c *limitOrderService) validateAuth() error {
	if c.apiKey == "" {
...
func (c *limitOrderService) authInterceptor() grpc.UnaryClientInterceptor {
