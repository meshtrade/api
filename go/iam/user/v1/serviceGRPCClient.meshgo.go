...
func (g *GRPCClientUserService) GetUser(ctx context.Context, request *GetUserRequest) (*User, error) {
	ctx, span := g.tracer.Start(
		ctx,
		UserServiceServiceProviderName+"GetUser",
	)
	defer span.End()

	// call given implementation of the adapted service provider interface
	getUserResponse, err := g.grpcClient.GetUser(ctx, request)
	if err != nil {
		log.Ctx(ctx).Error().Err(err).Msg("could not GetUser")
		return nil, fmt.Errorf("could not GetUser: %s", err)
	}
